...
def accept(
    ...
    """
    Accepts a value read from the standard input, optionally validating it with `check_fn` and converting it to another type with `convert_fn`. Note that the value will only be accepted if `check_fn` returns `True` and `convert_fn` successfully completes. Thus, `check_fn` should be a boolean function, while `convert_fn` should signal a failed conversion by raising an exception. `check_fn` is called before `convert_fn` and is applied directly to the string value read from stdin. Of course, you can implement all the validation logic in `convert_fn`, without ever needing to use `check_fn`.
